/* 99% of this code is taken from Len Brown's turbostat.c code
 * (see below) ... the only changes are minor tweaks to dump
 * the raw data instead of calculating the percentages/ghz numbers
 */
...
void get_counters(struct counters *cnt)
{
	for ( ; cnt; cnt = cnt->next) {
		...
		/* JBP this may not have been in original code */
		/* dump_cnt( cnt ); */
	}
}
...
void dump_vals( void ) {
	...
	printf("# cpu pkg core tsc aperf mperf c3 c6 c7 pc2 pc3 pc6 pc7\n");
	for (; cnt_even; cnt_even = cnt_even->next) {
		printf( "%d %d %d %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu\n",
				cnt_even->cpu, cnt_even->pkg, cnt_even->core,
				cnt_even->tsc, cnt_even->aperf, cnt_even->mperf,
				cnt_even->c3, cnt_even->c6, cnt_even->c7,
				cnt_even->pc2, cnt_even->pc3, cnt_even->pc6,
				cnt_even->pc7 );
	}
...
void cmdline(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "+vdi:M:")) != -1) {
		switch (opt) {
		case 'v':
			verbose++;
			break;
